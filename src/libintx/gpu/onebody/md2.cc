...
namespace libintx::gpu::md {
    ...
    double *V)
  {
    ...
    // Overlap is the only operator whose whole derivative fits this buffer.
    // The others get the same treatment `compute` gives Coulomb: say so,
    // rather than hand back a buffer that reads as a zero gradient -- which is
    // a plausible-looking answer and the reason a missing derivative term is
