...
// positive definiteness for the metric. What the scaffolding case at the bottom
// still checks is the engine around them: the factory, the batching invariant,
// the point-charge upload through set(), and that the (A|B) fallback table is
// now unreachable.
//
...
// sweep carries its own oracle -- see below.
...
TEST_CASE("libintx.gpu.md2.scaffolding") {
  ...
  SUBCASE("derivatives not implemented yet") {
    ...
    // error. Coulomb has a VALUE kernel now but no derivative one -- issue #28
    // deferred d(P|Q)/dX to whatever route issue #27 settles on -- so it still
    // belongs in this list even though it is gone from the one above.
