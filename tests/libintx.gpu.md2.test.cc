...
//
// Two operators additionally have a DERIVATIVE kernel, dS/dX and dV/dX, and
// their cases sit lower down. They carry their own oracle -- central finite
// differences of the same libintx::md::reference -- because there is no host
// derivative engine to compare against. For dV/dX the differences displace the
// NUCLEI as well as the shell centres: the electron-nuclear operator is the one
// here that moves with the geometry itself, and a sweep over shell centres
// alone passes with its Hellmann-Feynman term entirely absent.
//
    ...
    for (int B = 0; B <= LMAX; ++B) {
      ...
      SUBCASE(str("(",A,"|",B,")").c_str()) {
        for (auto K : Ks) {
          ...
          auto G = gradient(Overlap, basis, ijs, A, B, stream);

          for (size_t ij = 0; ij < ijs.size(); ++ij) {
            auto [i,j] = ijs[ij];
            for (int x = 0; x < 3; ++x) {
              auto ref = bra_gradient_reference(Overlap, basis[i], basis[j], None, x);
...
TEST_CASE("libintx.gpu.md2.Overlap.gradient.translation") {
  ...
  for (int A = 0; A <= LMAX; ++A) {
    for (int B = 0; B <= LMAX; ++B) {
      ...
      SUBCASE(str("(",A,"|",B,") + (",B,"|",A,")^T == 0").c_str()) {
        ...
        auto Gab = gradient(Overlap, basis, ab, A, B, stream);
        auto Gba = gradient(Overlap, basis, ba, B, A, stream);
  ...
  for (int L = 0; L <= LMAX; ++L) {
    ...
    SUBCASE(str("(",L,"|",L,") on one centre").c_str()) {
      ...
      auto G = gradient(Overlap, basis, ijs, L, L, stream);
...
TEST_CASE("libintx.gpu.md2.scaffolding") {
  ...
  SUBCASE("derivatives not implemented yet") {
    // Kinetic and Coulomb have no derivative kernel and must say so: a zero
    // gradient is a plausible-looking answer, and a caller assembling dE/dX
    // out of one gets a smooth, wrong force rather than an error.
    //
